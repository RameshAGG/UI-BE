import {Controller, Post, Body, Get, Param, Put, Delete, UseInterceptors, UploadedFile, Query} from '@nestjs/common';
...
export class VendorsController {
  ...
  @UseInterceptors(FileInterceptor('upload_file', {
    storage: diskStorage({
      destination: './uploads/vendorsFiles',
  ...
  create(@Body() data: any, @UploadedFile() file: Express.Multer.File) {
    ...
    return this.vendorsService.create({ ...data, upload_file: filePath });
  }

  @Get()
  async findAll(@Query('search') search?: string) {
    const vendors = await this.vendorsService.findAll(search);
    ...
    return vendors.map(vendor => ({
      ...
      sub_service_name: vendor.sub_service_name,
      sub_service_id: vendor.sub_service_id,
      joining_date: vendor.joining_date,
    }));
  }
...
update(
  ...
  return this.vendorsService.update(id, parsedData);
}
